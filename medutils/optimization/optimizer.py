...
from .prox import prox_p, prox_q, prox_denoise
...
class ICTVOptimizer(BaseOptimizer):
    ...
    """
    def __init__(self, mode, lambd, alpha1, s, beta1=None, beta2=None, prox_h=prox_denoise):
        super().__init__(mode, lambd)
        self.alpha1 = alpha1
        self.s = s
        assert 0 <= s <= 1
        self.beta1 = beta1
    ...
    def solve(self, y, max_iter):
        ...
        # setup dual variables
        p1 = np.zeros_like(K_beta1(y))
        p2 = np.zeros_like(K_beta1(y))
        ...
        for _ in tqdm.tqdm(range(max_iter)):
            ...
            Kx2 = K_beta2(x2)
            Kx_all = np.concatenate([Kx1.flatten(), Kx2.flatten()])
...
class ICTGVOptimizer(BaseOptimizer):
    ...
    https://doi.org/10.1002/mrm.26352
    """
    def __init__(self, mode, lambd, alpha0, alpha1, s, beta1=None, beta2=None, prox_h=prox_denoise):
        super().__init__(mode, lambd)
        self.alpha0 = alpha0
        self.alpha1 = alpha1
        self.s = s
    ...
    def solve(self, y, max_iter):
        ...
        for _ in tqdm.tqdm(range(max_iter)):
            ...
            Kx3 = K_beta2(x3) - x4
            Kx4 = E_beta2(x4)
            Kx_all = np.concatenate([Kx1.flatten(), Kx2.flatten(), Kx3.flatten(), Kx4.flatten()])
